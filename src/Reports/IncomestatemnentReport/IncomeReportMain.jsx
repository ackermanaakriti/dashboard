...
import IncomeStatementWebView from './IncomeStatementWebView';
import IncomePdfView from './IncomePdfView';


const IncomeReportMain = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.post(
                `${baseUrl}Ledger/GenerateIncomeStatementReport`,{startDate,endDate},
                {   
                    headers: {
    ...
    const handleDownload = async () => {
        const blob = await pdf(<IncomePdfView startDate={startDate} endDate={endDate} tableData={tableData}/>).toBlob();
        ...
        <div className='pb-[50px]'>
            <div className='w-full pb-[20px] border-b-[1px] border-b-[#ddd] border-b-solid'>
                <div className='flex justify-center items-center h-full'>
                    <h2 className='text-inter text-PrimaryColor text-[30px]'>Income Statement Report</h2>
                </div>
                ...
                {showWebView &&   <IncomeStatementWebView tableData={tableData} endDate={endDate} startDate={startDate}/>}
                </div>
...
export default IncomeReportMain;
