...
const BranchTable = () => {
  ...
  const {Deldata}= useDelData('Branch/Delete/')   //use custom delete hook
  const { data } = useGetData('Branch/GetAll')   //use custom hook to get all data...passing url
  ...
  const handleEdit = (id) => {
    console.log(id)
    setId(id);
    dispatch(addMenu({ id: id, menu: "branchForm" }));   // dispatched menu for component navigation..using object mapping for navigation so passing menu as key
  };
